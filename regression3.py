...
class Classifers():
    ...
    def ova_predict(self, classifier, pos_class):
        texas_train_preds = classifier.predict(self.texas_train)
        france_train_preds = classifier.predict(self.france_train)
        maine_train_preds = classifier.predict(self.maine_train)
        oregon_train_preds = classifier.predict(self.oregon_train)
        texas_val_preds = classifier.predict(self.texas_val)
        france_val_preds = classifier.predict(self.france_val)
        maine_val_preds = classifier.predict(self.maine_val)
        oregon_val_preds = classifier.predict(self.oregon_val)
        if pos_class == "texas":
            pos_train_preds = texas_train_preds
            neg_train_preds = np.concatenate((france_train_preds, maine_train_preds, oregon_train_preds))
            pos_val_preds = texas_val_preds
            neg_val_preds = np.concatenate((france_val_preds, maine_val_preds, oregon_val_preds))
        elif pos_class == "france":
            pos_val_preds = france_train_preds
            neg_val_preds = np.concatenate((texas_train_preds, maine_train_preds, oregon_train_preds))
            pos_val_preds = france_val_preds
            neg_val_preds = np.concatenate((texas_val_preds, maine_val_preds, oregon_val_preds))
        elif pos_class == "maine":
            pos_train_preds = maine_train_preds
            neg_train_preds = np.concatenate((texas_train_preds, france_train_preds, oregon_train_preds))
            pos_val_preds = maine_val_preds
            neg_val_preds = np.concatenate((texas_val_preds, france_val_preds, oregon_val_preds))
        elif pos_class == "oregon":
            pos_train_preds = oregon_train_preds
            neg_train_preds = np.concatenate((texas_train_preds, france_train_preds, maine_train_preds))
            pos_val_preds = oregon_val_preds
            neg_val_preds = np.concatenate((texas_val_preds, france_val_preds, maine_val_preds))
        else:
            raise Exception("Bad pos class")
        
        # Train
        train_true_pos = np.sum(pos_train_preds)
        train_false_pos = np.sum(neg_train_preds)
        train_true_neg = len(neg_train_preds) - train_false_pos
        train_false_neg = len(pos_train_preds) - train_true_pos
        train_accuracy = (train_true_pos+train_true_neg) / (train_true_pos+train_false_pos+train_true_neg+train_false_neg)
        train_precision = train_true_pos / (train_true_pos+train_false_pos)
        train_recall = train_true_pos / (train_true_pos+train_false_neg)

        # Validation
        val_true_pos = np.sum(pos_val_preds)
        val_false_pos = np.sum(neg_val_preds)
        val_true_neg = len(neg_val_preds) - val_false_pos
        val_false_neg = len(pos_val_preds) - val_true_pos
        val_accuracy = (val_true_pos+val_true_neg) / (val_true_pos+val_false_pos+val_true_neg+val_false_neg)
